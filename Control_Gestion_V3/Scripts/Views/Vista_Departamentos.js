...
$(function () {
    ...
    // Resetear formulario y clases al cerrar modal
    $('#FormModal').on('hidden.bs.modal', function () {
        const form = $('#form');
        form[0].reset();
        form.find('.is-valid, .is-invalid').removeClass('is-valid is-invalid');
        form.find('span.error, span.text-danger').remove(); // más robusto
    });
});
...
function Guardar() {
    if ($("#form").valid()) {
        var request = {
            objeto: {
                Id_Departamento: parseInt($("#txtid").val()) || 0,
        ...
        $.ajax({
            url: $.MisUrls.url._GuardarDepartamentos,
            type: "POST",
...
function eliminar(id) {
    swal({
        title: "Mensaje",
        text: "¿Desea eliminar el departamento seleccionado?",
        type: "warning",
        ...
        function () {
            jQuery.ajax({
                url: $.MisUrls.url._EliminarDepartamento + "?id=" + id,
                type: "GET",
                dataType: "json",
                contentType: "application/json; charset=utf-8",
                success: function (data) {
                    if (data.resultado) {
                        swal("Mensaje", "Departamento eliminado correctamente.", "success");
                        tabladata.ajax.reload();
                    } else {
                        setTimeout(function () {
                            swal("Mensaje", "No se pudo eliminar el departamento si está ligado a un caso", "warning");
                ...
                error: function (error) {
                    console.error(error);
                    swal("Mensaje", "Ocurrió un error al intentar eliminar el departamento.", "error");
                }
            });
        });
}
